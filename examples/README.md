...
| [`stunnel/`](stunnel/) (config templates)          | Mutual-TLS transport for the data socket when the reader can't speak SSH. sloth stays bound to loopback; stunnel owns the certificates and the handshake. Sensor-side and reader-side configs, plus the `accept`/`connect` mistake that fails open. |
