...
in [`dark-factory.md`](dark-factory.md); working rules in
...
./sloth --data-socket tcp:100.64.0.5:8765 --data-socket-allow-remote
                                         # JSONL over TCP. A non-loopback bind is
                                         # refused without the opt-in flag (#86);
                                         # the stream has no auth and no TLS.
```
...
### 10.3 Risk gate (converge loop step 6)

The converge loop consults `agents/risk_score.sh` before shipping: a
zero-deps POSIX-sh scorer that rates the pending diff (working tree vs
`origin/main`) for blast radius. The green suite answers "is it
correct"; the risk score answers "how much should a human look first".
...
| Churn > 300 lines (> 800) | +15 (+30) |
| docs/ + tests/ + `*.md` only (agent surface always hot) | forces 0 |

Threshold: `RISK_THRESHOLD` env, default **50**. Modes:

- **Advisory** (default): score is reported in the order's issue
  comment; the loop proceeds regardless.
- **Enforcing** (marker file `agents/risk_enforce` exists): at/above
  threshold the loop stops without shipping and surfaces the breakdown
  on the issue. Creating the marker is an owner decision — weights and
  threshold must be owner-reviewed first (issue #45 acceptance).
...
> diffs: their numstat lines were `-`, churn from them counted as zero,
> and only the +40 path signal fired. Now that they are plaintext,
> churn counts too — so an `agents/` edit scores 40 plus its churn tier
> and can cross the 50 threshold on its own. That is the correct
> behaviour (a large rewrite of the rules governing agents *is* riskier
> than a one-line fix) but it means historical scores for
> instruction-file commits are not comparable with new ones.

### 10.4 Add a platform op
...
| The dark-factory pattern itself | [`dark-factory.md`](dark-factory.md) |
